...
/**
 * @file find_mincut_splitting.hpp
 *
 * @brief Find Min-Cut in AIG graph based on nodes and splitting
 *
 * @author Mathias Soeken
 * @since  2.0
 */

#ifndef FIND_MINCUT_SPLITTING_HPP
#define FIND_MINCUT_SPLITTING_HPP
...
namespace cirkit
{
   ...
   */
  bool find_mincut_splitting( std::list<std::list<aig_node>>& cuts, aig_graph& aig, unsigned count,
                              properties::ptr settings = properties::ptr(), properties::ptr statistics = properties::ptr() );

  mincut_by_node_func find_mincut_splitting_func( properties::ptr settings = properties::ptr( new properties ),
