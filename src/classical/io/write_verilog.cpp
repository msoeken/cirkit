...
{
  if ( f.second && boost::find( inverter_created, f.first ) == inverter_created.end() )
  {
    os << format( "not %1%_inv( %1%_inv, %1%" ) % get_node_name_processed( f.first, aig, &remove_brackets ) << std::endl;
    inverter_created += f.first;
  }
}
...
void write_verilog( const aig_graph& aig, std::ostream& os )
{
  ...
  for ( const auto& v : boost::make_iterator_range( boost::vertices( aig ) ) )
  {
    ...
    os << format( "and %1%( %1%, %2%%3%, %4%%5% )" ) % get_node_name_processed( v, aig, &remove_brackets )
      % get_node_name_processed( operands.first.first, aig, &remove_brackets ) % ( operands.first.second ? "_inv" : "" )
      % get_node_name_processed( operands.second.first, aig, &remove_brackets ) % ( operands.second.second ? "_inv" : "" ) << std::endl;
  }

  /* Output functions */
  for ( const auto& v : aig_info.outputs )
  {
    os << format( "%1% %2%( %2%, %3% )" ) % ( v.first.second ? "not" : "buf" )
                                          % remove_brackets( v.second )
                                          % get_node_name_processed( v.first.first, aig, &remove_brackets )
       << std::endl;
  }
