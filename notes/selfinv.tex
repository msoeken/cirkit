...
\smallskip \noindent{\bf Lemma 2.1.} The class $T_i$, $i \in \{1,...,n-1\}$, is equal to the set of transpositions $(a\,b)$ where 
$$
\{(a\,b) \,|\, a = a_1 + a_2 \cdot 2^i, a_1 \in \{0,..,2^{i-1}-1\}, a_2 \in \{0,..,2^{n-i}-1\}, length (a\,b) = 2^{i-1} \}
$$

\smallskip\noindent {\it Proof.} MKT: There must be an easier way to define this. Also perhaps it should be a definition instead of a lemma.\qquad\slug
...
\smallskip\noindent {\bf Example 1.} See the other paper for examples for $n$ equal to 2 and 3. 
